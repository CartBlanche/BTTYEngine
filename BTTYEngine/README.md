![BTTY Engine - Logo](btty-engine.png)

# BTTYEngine Developer Guide

BTTYEngine is a simple voxel game engine that is built on [MonoGame](https://monogame.net/).

 [`VoxelShooter/README.md`](../VoxelShooter/README.md) is the first demo, but the engine is (hopefully) designed to be used by any MonoGame project via a `<ProjectReference>`.
...
|-------|-----------|----------|
| `SideScrollingCamera` | Orthographic | Classic side-scroller |
| `IsometricCamera` | Orthographic | 45° isometric view |
| `FirstPersonCamera` | Perspective | First-person |
...
The shake decays at 15% per frame and won't cancel an in-progress shake. It takes the maximum of the two. The shake should work through transitions too.

---

## Input

`InputManager<TAction>` is a game-agnostic input system, inspired by SDL3's. 
The engine never hardcodes a single key, your game defines its own action enum and builds a binding table. Any combination of keyboard keys, gamepad buttons, gamepad analogue axes, mouse buttons, mouse analogue axes, and touch gestures can be bound to the same action.
...
BTTYEngine wraps [BepuPhysics 2.4.0](https://github.com/bepu/bepuphysics2) behind `PhysicsManager`, which runs single-threaded, which is fine for smaller games. I may look into making it multi-threaded down the line, if more complex games require it. 
...
BEPU callbacks run on the main thread (for now). BTTYEngine queues collision pairs and lets you process them safely on the main thread via `CollisionEventHandler`:
...
`VoxelWorld` is a 3-D grid of `Chunk` objects. Each chunk builds VertexArray/IndexArray s and draws them via DrawUserIndexedPrimitives; the game loop asks for `BoundingFrustum.Intersects(chunk.boundingSphere)` before drawing, so off-screen chunks cost nothing.
...
`ParticleController` manages a pool of `ParticleCube` instances that fly out on impact, fade, and return to the pool automatically.
